...
using DapperMapper.Caching.Interfaces;
using System;
...
namespace DapperMapper.Caching
{
    public sealed class CachingMannager : ICachingProvider
    {
        ...
        public void Adicionar<T>(string chave, T valor)
        {
            lock (Padlock)
            {
                ...
                OnPropertyChanged(chave, ActionCache.Adicionado);
            }
        }
        ...
        public void Remover(string chave)
        {
            ...
            {
                _cache.Remove(chave);
                OnPropertyChanged(chave, ActionCache.Removido);
            }
        }
        ...
        public KeyValuePair<bool, T> Recuperar<T>(string chave, bool removerAposRecuperar = false)
        {
            ...
            return new KeyValuePair<bool, T>(retorno.Key, retorno.Value != null ? (T)retorno.Value : default);
        }
        ...
        {
            OnPropertyChanged(arguments.CacheItem.Key, ActionCache.Expirou);
        }
